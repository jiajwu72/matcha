    ...
    <v-card class="overflow-hidden">
      <v-app-bar app dark id="header">
        ...
        <v-toolbar-items v-if="isMobile==false" >
            ...
            <v-btn v-for="item in rightBarInfo"
                    :key="item.icon" :to="item.link"
                    style="background-color:rgba(0, 0, 0,0);"
                    @click="dirige(item.tag)"
  ...
  data() {
    return {
      ...
      rightBarInfo: [
          ...
          { title: 'Espace Client', icon: 'mdi-account',tag:'clientSpace' },
          // { title: 'Panier', icon: 'mdi-cart',tag:'shoppingCard' },
        ],
      action:"",
    };
